...
//  通过method获取我到我们调用什么样的方法名，将我们实现构造的request字节序写进为req_protocol协议的data部分，然后发送出去，
void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
  ...
  s_ptr channel = shared_from_this();   //  获得指向自己的一个智能指针，防止直接用this指针的话被析构导致出错

    //  定时事件，设置超时任务，超时之后提示超时，同时回收资源
  ...
  m_client->addTimerEvent(timer_event); //  添加定时事件
  ...
  m_client->connect([req_protocol, this]() mutable {
    ...
    //  连接成功后，将请求的协议对象发送过去。对可写事件就绪进行监听，当可写时会调用connection类中的onwrite函数，然后把message转化为协议字符串发送出去，同时后面会执行下文的函数流程
