...
TcpConnection::TcpConnection(EventLoop* event_loop, int fd, int buffer_size, NetAddr::s_ptr peer_addr, NetAddr::s_ptr local_addr, TcpConnectionType type /*= TcpConnectionByServer*/)
  ...
  m_in_buffer = std::make_shared<TcpBuffer>(buffer_size); //  接收缓冲区
  m_out_buffer = std::make_shared<TcpBuffer>(buffer_size);  //  发送缓冲区
...
void TcpConnection::onRead() {
  ...
  while(!is_read_all) {
    ...
    int read_count = m_in_buffer->writeAble();  //  获得可写入空间的大小
    int write_index = m_in_buffer->writeIndex();    //  获得写入位置的index

    int rt = read(m_fd, &(m_in_buffer->m_buffer[write_index]), read_count); //  将读取的数据存储到可写入的位置
    DEBUGLOG("success read %d bytes from addr[%s], client fd[%d]", rt, m_peer_addr->toString().c_str(), m_fd);
    if (rt > 0) {
      m_in_buffer->moveWriteIndex(rt);  //  表示读取了rt个字节
      if (rt == read_count) {   //  说明很有可能有数据没有读取完，继续读
        continue;
      ...
      is_close = true;
      break;
    } else if (rt == -1 && errno == EAGAIN) { //  表示已经读完了
      is_read_all = true;
      break;
    }
  }
...
void TcpConnection::excute() {
  if (m_connection_type == TcpConnectionByServer) {
    ...
    m_coder->decode(result, m_in_buffer); //  将in_buffer中的message读取出来存储在result中
...
void TcpConnection::onWrite() {
  ...
  if (m_connection_type == TcpConnectionByClient) {
    ...
    m_coder->encode(messages, m_out_buffer);  //  将消息解码写入缓冲区
  }
  ...
  //  因为处于LT模式，所以在写完之后如果不将事件清除或者直接写完，就会一直触发
  if (is_write_all) {
  ...
  if (m_connection_type == TcpConnectionByClient) {
    //  m_write_dones的message在前面while循环中已经全部发送，此处是执行其done函数，可能是得到反馈
    for (size_t i = 0; i < m_write_dones.size(); ++i) {
