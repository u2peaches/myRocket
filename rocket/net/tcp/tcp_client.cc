...
TcpClient::TcpClient(NetAddr::s_ptr peer_addr) : m_peer_addr(peer_addr) {
  m_event_loop = EventLoop::GetCurrentEventLoop();  //  获取主循环的EventLoop
  ...
  m_fd_event = FdEventGroup::GetFdEventGroup()->getFdEvent(m_fd);   //  获取fd_event对象
  m_fd_event->setNonBlock();

  m_connection = std::make_shared<TcpConnection>(m_event_loop, m_fd, 128, peer_addr, nullptr, TcpConnectionByClient);
  m_connection->setConnectionType(TcpConnectionByClient);   //  设置连接主体类型
...
void TcpClient::connect(std::function<void()> done) {
  int rt = ::connect(m_fd, m_peer_addr->getSockAddr(), m_peer_addr->getSockLen());  //  发起连接，m_peer_addr是目标地址
  if (rt == 0) {
    ...
    if (errno == EINPROGRESS) {
      // epoll 监听可写事件，然后判断错误码，继续尝试连接
      m_fd_event->listen(FdEvent::OUT_EVENT,
        [this, done]() {
          int rt = ::connect(m_fd, m_peer_addr->getSockAddr(), m_peer_addr->getSockLen());  //  获取连接
          if ((rt < 0 && errno == EISCONN) || (rt == 0)) {  //  判断是否连接成功
            DEBUGLOG("connect [%s] sussess", m_peer_addr->toString().c_str()); 
            initLocalAddr();
            m_connection->setState(Connected);
          } else {
      ...
      m_event_loop->addEpollEvent(m_fd_event);  //  添加事件

      if (!m_event_loop->isLooping()) { //  判断是否在loop
        m_event_loop->loop();
      }
    } else {
      ERRORLOG("connect errror, errno=%d, error=%s", errno, strerror(errno));
