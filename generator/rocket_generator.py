...
def parseInput():
    ...
    for opts,arg in opts:
        ...
        if opts=="-h" or opts=="--help":    #   如果打印的入参是-h或者--help则表明需要帮助
            printHelp()
            sys.exit(0)
        if opts=="-i" or opts=="--input":   #   -i表示访问protobuf文件，将protobuf文件取出来赋值
            global proto_file 
            proto_file = arg 
        elif opts=="-o" or opts=="--output_path":   #   指定输出路径，赋值到某个参数
    ...
    global project_name
    project_name = proto_file[0: -6]    #   获取文件名，将后面六位去掉，也就是把.proto去掉
...
def generate_project():
    ...
    try:
        ...
        generate_pb()   # 生成pb文件目录

        generate_makefile() #   生成makefile文件
